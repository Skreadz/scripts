...
$serveur = Read-host "Saisir le nom DNS du premier serveur de fichier"
$cheminouprincipale = "OU=$ou_principale,$cheminbase"
$cheminuser = "OU=SERVICES,"+$cheminouprincipale
$chemingroupe = "OU=GROUPES,"+$cheminouprincipale
$cheminservices = "\\"+$serveur+"\partage\SERVICES\"
$cheminperso = "\\"+$serveur+"\partage\PERSO\"
$ou_default = @("GROUPES","SERVICES","ORDINATEURS")
...
function user {         #### Fonction pour créer utilisateur
    ...
    ForEach ($line in $csv) {
        ...
        $mail = $upn
        $group = $line.ou.toUpper()
        $cheminusergroup = "OU="+$group+",OU=UTILISATEURS,"+$cheminouprincipale
        $prenom = $line.prenom.toLower()
        $nom = $line.nom.toLower()

        New-ADUser -Name $nom -GivenName $prenom -Surname $nom -SamAccountName $sam -UserPrincipalName $upn -EmailAddress $mail -Enable $true -ChangePasswordAtLogon $true -Path $cheminusergroup -Verbose -AccountPassword (convertto-securestring "Azerty1+" -asplaintext -force)
    }
}
...
function acl_user {
    ...
    ForEach ($line in $csv) {
        $sam = $line.prenom.toLower()+"."+$line.nom.toLower()
        $service = "\\"+$serveur+"\partage\PERSO\"+$sam
        $group = $line.group.toUpper()      #### Récupère la section group du CSV en plus de l'écrire en majuscules
        $GG="GG_"+$group
...
function acl_services {
    ...
    ForEach ($line in $csv) {
        $ou = $line.ou.toUpper() 
        $service = "\\"+$serveur+"\partage\SERVICES\"+$ou
        $group = $line.group.toUpper()                  #### Récupère la section group du CSV en plus de l'écrire en majuscules
        $GDL="GDL_"+$group+"_RW"
